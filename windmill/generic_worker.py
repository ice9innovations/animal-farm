...
class WorkerConfig:
    ...
    def __init__(self, env_file='.env'):
        ...
        self.enable_triggers = os.getenv('ENABLE_TRIGGERS', 'true').lower() == 'true'
...
class GenericWorker:
    ...
    def trigger_post_processing(self, image_data, result):
        """Post-processing is triggered automatically by timestamp comparison"""
        # No explicit triggering needed! The workers detect new results automatically:
        # 
        # - bbox_merger_worker: Detects when merged_boxes.created < MAX(results.result_created)
        # - consensus_worker: Detects when consensus.created < MAX(results.result_created)
        # 
        # This is much cleaner than manual triggers and handles edge cases better.
        pass

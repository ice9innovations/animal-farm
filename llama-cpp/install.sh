#!/bin/bash
# Install llama-cpp Flask API dependencies into llama-cpp/venv.
...
#   bash run.sh  (RunPod)
#   systemctl start llama-cpp  (systemd)
set -e
...
bash "$(dirname "$(realpath "$0")")/build_server.sh"
...
[Unit]
Description=Animal Farm llama.cpp Vision Service

...
# Install GPU-first dependencies into pose/venv
bash install.sh
```
...
| `POSE_BACKEND` | No | `auto` | `auto`, `onnx`, and `trt` use BlazePose ONNX. `mediapipe` is explicit compatibility mode only. |
...
TensorRT can spend minutes building engines at startup, especially on Jetson. Engines are cached under `models/trt_cache`, but the normal GPU default is CUDA-first to keep service startup fast. Use `ONNX_PROVIDER_ORDER=tensorrt,cuda,cpu` only when TensorRT is installed and startup/build latency is acceptable for that host.
...
```bash
docker build --build-arg ORT_PACKAGE=onnxruntime-gpu==1.23.2 -t animal-farm-pose ./pose
```
...
The systemd unit is generated by [install.sh](/home/sd/animal-farm/pose/install.sh), using the current checkout path as `WorkingDirectory`, `EnvironmentFile`, and `ExecStart`.

Typical flow:

```bash
bash install.sh
sudo cp pose.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl start pose
```

If you deploy the service somewhere else, rerun the installer from that location so the generated unit points at the correct paths.
...
# Enable segmentation only if you plan to consume the mask output internally
ENABLE_SEGMENTATION=true

# Keep segmentation disabled for lower overhead
...
- Keep `ENABLE_SEGMENTATION=false` unless you explicitly need MediaPipe body segmentation

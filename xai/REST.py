...
def load_emoji_mappings():
    ...
    if AUTO_UPDATE:
        try:
            logger.info(f"🔄 xAI: Loading emoji mappings from GitHub: {github_url}")
            response = requests.get(github_url, timeout=10.0)
            response.raise_for_status()
            with open(local_cache_path, 'w', encoding='utf-8') as f:
                json.dump(response.json(), f, indent=2, ensure_ascii=False)
            logger.info("✅ xAI: Loaded emoji mappings from GitHub and cached locally")
            return response.json()
        except requests.exceptions.RequestException as e:
            logger.warning(f"⚠️  xAI: Failed to load emoji mappings from GitHub ({e}), falling back to local cache")

    try:
        with open(local_cache_path, 'r') as f:
            local_mappings = json.load(f)
            logger.info("✅ xAI: Successfully loaded emoji mappings from local cache")
            return local_mappings
    except Exception as local_error:
        logger.error(f"❌ xAI: Failed to load local emoji mappings: {local_error}")
...
def load_mwe_mappings():
    ...
    if AUTO_UPDATE:
        try:
            logger.info(f"🔄 xAI: Loading MWE mappings from GitHub: {github_url}")
            response = requests.get(github_url, timeout=10.0)
            response.raise_for_status()
            with open(local_cache_path, 'w') as f:
                f.write(response.text)
            mwe_tuples = process_mwe_lines(response.text.splitlines())
            logger.info("✅ xAI: Loaded MWE mappings from GitHub and cached locally")
            return mwe_tuples
        except requests.exceptions.RequestException as e:
            logger.warning(f"⚠️  xAI: Failed to load MWE mappings from GitHub ({e}), falling back to local cache")

    try:
        with open(local_cache_path, 'r') as f:
            local_mwe_text = f.read().splitlines()
            mwe_tuples = process_mwe_lines(local_mwe_text)
            logger.info(f"✅ xAI: Successfully loaded {len(mwe_tuples)} multi-word expressions from local cache")
            return mwe_tuples
    except Exception as local_error:
        logger.error(f"❌ xAI: Failed to load local MWE mappings: {local_error}")
...
def lookup_text_for_emojis(text: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"xAI: Failed to process emoji mappings: {e}")
        return {"mappings": {}, "found_emojis": []}
...
def process_image_with_xai(image: Image.Image, prompt: str = None) -> Dict[str, Any]:
    """Send a PIL Image to xAI Grok for vision analysis via the OpenAI-compatible API."""
...
def create_response(data: Dict[str, Any], processing_time: float) -> Dict[str, Any]:
    ...
    if is_shiny:
        ...
        logger.info(f"✨ SHINY xAI VISION ANALYSIS! Roll: {shiny_roll} ✨")
...
def analyze():
    ...
    try:
        ...
        processing_result = process_image_with_xai(image, prompt)
...
if __name__ == '__main__':
    logger.info("Starting xAI Grok vision service...")

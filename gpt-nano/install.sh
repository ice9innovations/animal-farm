#!/bin/bash
# Install GPT Nano Flask service dependencies into gpt-nano/venv.
...
#   bash run.sh  (RunPod)
#   systemctl start gpt-nano  (systemd)
set -e

SCRIPT_DIR="$(dirname "$(realpath "$0")")"
SERVICE_NAME="gpt-nano"
CURRENT_USER="$(whoami)"
...
[Unit]
Description=Animal Farm GPT Nano Vision Service

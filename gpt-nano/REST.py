...
def load_emoji_mappings():
    ...
    if AUTO_UPDATE:
        try:
            logger.info(f"🔄 GPT Nano: Loading emoji mappings from GitHub: {github_url}")
            response = requests.get(github_url, timeout=10.0)
            response.raise_for_status()
            with open(local_cache_path, 'w', encoding='utf-8') as f:
                json.dump(response.json(), f, indent=2, ensure_ascii=False)
            logger.info("✅ GPT Nano: Loaded emoji mappings from GitHub and cached locally")
            return response.json()
        except requests.exceptions.RequestException as e:
            logger.warning(f"⚠️  GPT Nano: Failed to load emoji mappings from GitHub ({e}), falling back to local cache")

    try:
        with open(local_cache_path, 'r') as f:
            local_mappings = json.load(f)
            logger.info("✅ GPT Nano: Successfully loaded emoji mappings from local cache")
            return local_mappings
    except Exception as local_error:
        logger.error(f"❌ GPT Nano: Failed to load local emoji mappings: {local_error}")
...
def load_mwe_mappings():
    ...
    if AUTO_UPDATE:
        try:
            logger.info(f"🔄 GPT Nano: Loading MWE mappings from GitHub: {github_url}")
            response = requests.get(github_url, timeout=10.0)
            response.raise_for_status()
            with open(local_cache_path, 'w') as f:
                f.write(response.text)
            mwe_tuples = process_mwe_lines(response.text.splitlines())
            logger.info("✅ GPT Nano: Loaded MWE mappings from GitHub and cached locally")
            return mwe_tuples
        except requests.exceptions.RequestException as e:
            logger.warning(f"⚠️  GPT Nano: Failed to load MWE mappings from GitHub ({e}), falling back to local cache")

    try:
        with open(local_cache_path, 'r') as f:
            local_mwe_text = f.read().splitlines()
            mwe_tuples = process_mwe_lines(local_mwe_text)
            logger.info(f"✅ GPT Nano: Successfully loaded {len(mwe_tuples)} multi-word expressions from local cache")
            return mwe_tuples
    except Exception as local_error:
        logger.error(f"❌ GPT Nano: Failed to load local MWE mappings: {local_error}")
...
def lookup_text_for_emojis(text: str) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.error(f"GPT Nano: Failed to process emoji mappings: {e}")
        return {"mappings": {}, "found_emojis": []}
...
def process_image_with_gpt(image: Image.Image, prompt: str = None) -> Dict[str, Any]:
    """Send a PIL Image to GPT-5 Nano for vision analysis via the OpenAI API."""
    ...
    try:
        ...
        response = client.chat.completions.create(
            ...
            messages=[
                {
                    "role": "system",
                    "content": "Respond with plain text only. No markdown, no headers, no bullet points.",
                },
                {
                    "role": "user",
                    "content": [
...
def create_response(data: Dict[str, Any], processing_time: float) -> Dict[str, Any]:
    ...
    if is_shiny:
        ...
        logger.info(f"✨ SHINY GPT NANO VISION ANALYSIS! Roll: {shiny_roll} ✨")
...
def analyze():
    ...
    try:
        ...
        processing_result = process_image_with_gpt(image, prompt)
...
if __name__ == '__main__':
    logger.info("Starting GPT Nano vision service...")

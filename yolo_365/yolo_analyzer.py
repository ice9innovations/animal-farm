...
class YoloAnalyzer:
    ...
    def _load_emoji_mappings(self) -> bool:
        """Load emoji mappings from local 365.json file"""
        try:
            with open('365.json', 'r') as f:
                self.emoji_mappings = json.load(f)
            logger.info(f"✅ {self.service_name}: Loaded emoji mappings from 365.json ({len(self.emoji_mappings)} entries)")
            return True
        except Exception as e:
            logger.error(f"❌ {self.service_name}: Failed to load 365.json: {e}")
            self.emoji_mappings = {}
            return False
